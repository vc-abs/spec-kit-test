...
### II. Iterative Validation and Quality Assurance

All new features and bugfixes MUST follow a validation → implementation → verification cycle, iterating until conclusion. For executable code, tests are written before implementation where feasible. For content generation and template-driven systems, validation focuses on operational behavior and quality gates rather than unit-level code coverage. No feature is merged without demonstrated quality validation appropriate to its type.
*Rationale: Guarantees reliability and quality while accommodating diverse system types including content generation workflows targeting non-technical users.*

### III. CLI and Text Protocols

All executable programs and libraries MUST expose functionality via a CLI interface. Input/output must use plain text or JSON over stdin/stdout, with errors to stderr. Human-readable and machine-readable formats are both required. Template-driven workflows and agent systems that operate through GitHub Copilot are exempt from CLI requirements.
*Rationale: Maximizes composability, debuggability, and automation for executable programs while acknowledging template-driven systems use different interaction models.*
...
- Content directories MUST maintain organizational structure appropriate to the project context (examples: prompts/, cards/, insta-prompts/, or feature-based organization like content/`<feature>`/, or type-based like content/`<asset-type>`/)
